...
export default function Header({ search, toggleSearch }) {
  const router = useRouter();
  const { push, pathname } = router;

  console.log("push pathname", typeof pathname);
  return (
    <header>
      ...
      <div className="headerBar__box">
        <div className="icon">
          {/* <div {pathname === "/" ? {className="icon"} : {className="icon unavailable"}}> */}
          <Link
            className={pathname !== "/" && "unavailable"}
            href="/"
            target="_self"
            alt="Toggle search bar"
            onClick={(event) => {
        ...
        <div className="icon unavailable">
          <Link href="/shoppingcard" target="_self" alt="Link to shopping card">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
